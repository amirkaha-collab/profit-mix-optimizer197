...
def _match_alt_item_row(item: Dict, df_ref: pd.DataFrame) -> Optional[pd.Series]:
    ...
    if fund:
        m = df_ref[df_ref["fund"].astype(str).str.strip() == fund]
        if track:
            mt = m[m["track"].astype(str).str.strip() == track]
            if not mt.empty:
                return mt.iloc[0]
        if not m.empty:
            return m.iloc[0]

    if track and manager:
        m = df_ref[
            (df_ref["track"].astype(str).str.strip() == track) &
            (df_ref["manager"].astype(str).str.strip() == manager)
        ]
        if not m.empty:
            return m.iloc[0]

    if track:
        m = df_ref[df_ref["track"].astype(str).str.strip() == track]
        if not m.empty:
            return m.iloc[0]
    return None


def _weighted_metric_from_items(items, df_ref: pd.DataFrame, metric_key: str) -> float:
    if not items or df_ref is None or df_ref.empty or metric_key not in df_ref.columns:
        return np.nan
    weighted_sum = 0.0
    weight_sum = 0.0
    for item in items:
        try:
            pct = float(str(item.get("pct", "") or "").replace("%", "").strip())
        except Exception:
            continue
        if pct <= 0:
            continue
        row = _match_alt_item_row(item, df_ref)
        if row is None:
            continue
        val = _to_float(row.get(metric_key, np.nan))
        if math.isnan(val):
            continue
        weighted_sum += pct * val
        weight_sum += pct
    if weight_sum <= 0:
        return np.nan
...
def _compute_weighted_returns_for_items(items, df_ref: pd.DataFrame) -> Dict[str, float]:
    return {
        "ret_ytd": _weighted_metric_from_items(items, df_ref, "ret_ytd"),
        "ret_12m": _weighted_metric_from_items(items, df_ref, "ret_12m"),
        "ret_36m": _weighted_metric_from_items(items, df_ref, "ret_36m"),
        "ret_60m": _weighted_metric_from_items(items, df_ref, "ret_60m"),
    }
...
def render_results_table(rows, baseline, voting_configured, df_ref=None):
    if not rows:
        return
    ...
    header_cells = "".join(f"<th>{h}</th>" for h in [
        "לחץ לבחירה ▼", "מסלולים / מנהל / משקל", "מניות", 'חו"ל', 'מט"ח', "לא-סחיר", "תשואה 12 חו׳", "שארפ", "שירות ואיכות"
    ])
  ...
  <td class='num'>{fmt_val(rrow,'ret_12m_weighted')}</td>
  {sharpe_cell}
...
<div style='font-size:13px;font-weight:800;color:#1F3A5F;margin-bottom:8px'>
  📊 {_esc(selected_alt)} — פרטים
            ...
            weighted_returns = sel_row.get("weighted_returns") or _compute_weighted_returns_for_items(items, df_ref)
            returns_rows = [
                ("תשואה מתחילת השנה", weighted_returns.get("ret_ytd")),
                ("תשואה מצטברת 12 חודשים אחרונים", weighted_returns.get("ret_12m")),
                ("תשואה מצטברת 36 חודשים אחרונים", weighted_returns.get("ret_36m")),
                ("תשואה מצטברת 60 חודשים אחרונים", weighted_returns.get("ret_60m")),
            ]
            returns_df = pd.DataFrame(
                [{"מדד": lbl, "תשואה משוקללת": _fmt_pct(val, 2) if not pd.isna(val) else "—"} for lbl, val in returns_rows]
            )
            st.markdown("**תשואות משוקללות — החלופה הנבחרת**")
...
if res and recs:
    ...
    for _key, rrow, title in [
        ...
        r["weighted_returns"] = _compute_weighted_returns_for_items(r["weights_items"], df_active)
        r["ret_12m_weighted"] = r["weighted_returns"].get("ret_12m")
        rows_list.append(r)
    ...
    # Results table
    render_results_table(rows_list, baseline, voting_conf, df_active)
